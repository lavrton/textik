...
var makeEditCommand = function (doc, start, count, words) {
  var selStart = doc.selection.start,
    selEnd = doc.selection.end;
  return function (log) {
    doc._wordOrdinals = [];
    var oldWords = Array.prototype.splice.apply(
      doc.words,
      [start, count].concat(words)
    );
...
var makeTransaction = function (perform) {
  ...
  return function (outerLog) {
    outerLog(
      makeTransaction(function (innerLog) {
...
var prototype = node.derive({
  ...
  insert: function (text, takeFocus) {
    this.select(
      this.selection.end + this.selectedRange().setText(text),
      null,
      takeFocus
    );
  },
  ...
  runs: function (emit, range) {
    var startDetails = this.wordContainingOrdinal(Math.max(0, range.start)),
      endDetails = this.wordContainingOrdinal(
      ...
      if (runFilters) {
        ...
        try {
          for (;;) {
            ...
            if (
              !self.editFilters.some(function (filter) {
                filter(self);
  ...
  getCaretCoords: function (ordinal) {
    var node = this.byOrdinal(ordinal),
      b;
    if (node) {
